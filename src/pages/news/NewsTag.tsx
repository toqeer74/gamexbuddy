...
export default function NewsTag(){
  ...
  const items = useMemo(() => (data as Item[]).filter(i => (i.tags||[]).includes(tag || ""))
    .sort((a,b) => +new Date(b.date) - +new Date(a.date)), [tag]);

  return (
    <section className="section">
      <div className="wrap">
        <nav style={{ fontSize: 13, opacity: .8, marginBottom: 10 }}>
          <Link to="/">Home</Link> / <Link to="/news">News</Link> / <span>#{tag}</span>
        </nav>
        <h1 className="h2">News tagged #{tag}</h1>
        <div className="grid">
          {items.map(n => (
